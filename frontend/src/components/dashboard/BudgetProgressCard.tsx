...
export default function BudgetProgressCard({ budget }: { budget: BudgetProgress }) {
  let progressColor = "var(--ui-main)"
  
  if (budget.status === "WARNING") {
    progressColor = "#f59e0b" // amber
  } else if (budget.status === "LIMIT_REACHED" || budget.status === "EXCEEDED") {
    ...
    <div className="mb-4">
      <div className="d-flex justify-content-between align-items-end mb-2">
        ...
        <div className="small text-muted">
          Spent: <span className="fw-semibold text-body">{fmt(budget.spent)}</span> / Limit: {fmt(budget.monthly_limit)}
        </div>
      </div>
      
      <div className="progress" style={{ height: 8, backgroundColor: "var(--bs-tertiary-bg)" }}>
        <div
